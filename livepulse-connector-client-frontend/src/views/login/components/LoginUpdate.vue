...
const onUpdate = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(valid => {
    if (valid) {
      const { phone, verifyCode, password, confirmPassword } = ruleForm;
      const { checkSmsForPhone, resetPassword } = useUserStoreHook();
      resetPassword({ phone, password, confirmPassword })
        .then(res2 => {
