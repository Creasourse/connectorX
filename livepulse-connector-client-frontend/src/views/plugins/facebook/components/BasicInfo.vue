  ...
  <div class="config-card">
    ...
    >
      <el-form-item label="连接器名称">
        <el-input
          :value="facebookConnectorName"
          disabled
          placeholder="Facebook连接器"
        />
      </el-form-item>
    ...
    <div class="permission-info">
      ...
      <ul class="permission-list">
        <li>App ID 和 App Secret 请在 Facebook Developers 后台获取</li>
        <li>Page ID 是你要管理的 Facebook 公共主页的唯一标识</li>
        <li>Access Token 需要具备相应的权限才能正常使用</li>
        <li>请妥善保管 App Secret 和 Access Token，泄露可能导致数据安全问题</li>
      </ul>
    </div>
...
// 连接器名称（固定）
const facebookConnectorName = "Facebook连接器";

// 表单数据
const formData = reactive({
  connectorName: "Facebook连接器",
  displayName: "",
  appId: "",
  appSecret: "",
  pageId: "",
...
const formRules: FormRules = {
  ...
  pageId: [{ required: true, message: "请输入Page ID", trigger: "blur" }],
  accessToken: [
...
const handleSave = async () => {
  ...
  try {
    await formRef.value.validate();
    const { displayName, appId, appSecret, pageId, accessToken, remark } =
      formData;
    const params = {
      connectorName: "Facebook连接器",
      displayName,
