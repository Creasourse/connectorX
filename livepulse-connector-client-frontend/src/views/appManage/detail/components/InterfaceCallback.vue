<template>
  <div class="event-callback-page">
    <div class="config-card">
      <h2 class="config-title">接口回调记录</h2>
      ...
      >
        ...
        <el-table-column prop="eventCode" label="事件编码" min-width="120" />
        <el-table-column
          prop="requestBody"
          label="请求参数"
          min-width="240"
        ...
        <el-table-column label="状态" width="100" align="center">
          <template #default="{ row }">
            <el-tag v-if="row.processStatus === 1" type="success" size="small">
              成功
            </el-tag>
            <el-tag
              v-else-if="row.processStatus === 2"
              type="danger"
              size="small"
            >
              失败
    ...
    >
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="状态">
          <el-tag
            v-if="currentDetail.processStatus === 1"
            type="success"
            size="small"
          >
            成功
          ...
          <el-tag
            v-else-if="currentDetail.processStatus === 2"
            type="danger"
            size="small"
          >
            失败
...
import { getInterfaceLogList } from "@/api/wechat";
...
const getList = () => {
  loading.value = true;
  getInterfaceLogList({ ...form, wecomCorpId: wecomCorpId.value })
    .then(res => {
      if (res.success) {
